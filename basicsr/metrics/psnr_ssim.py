...
def calculate_psnr(img1,
    ...
    def _psnr(img1, img2):
        ...
        if Mask is not None:
            mse = np.sum(((img1 - img2)*Mask)**2)/np.sum(Mask)
        else:
...
def _ssim_3d(img1, img2, max_value, Mask=None):
...
def calculate_ssim(img1,
    ...
    def _cal_ssim(img1, img2):
        ...
        max_value = 1 if img1.max() <= 1 else 255
        with torch.no_grad():
            final_ssim = _ssim_3d(img1, img2, max_value, Mask=Mask) if ssim3d else _ssim(img1, img2, max_value)
