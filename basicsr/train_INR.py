...
def main():
    ...
    train_loader, val_loader = create_INR_dataloader(opt, logger)

    # create message logger (formatted outputs)
    msg_logger = MessageLoggerINR(opt, start_iter=1, tb_logger=tb_logger)

    # dataloader prefetcher
    prefetch_mode = opt['datasets']['val'].get('prefetch_mode')
    if prefetch_mode is None or prefetch_mode == 'cpu':
        prefetcher = CPUPrefetcher(train_loader)
        val_prefetcher = CPUPrefetcher(val_loader)
    elif prefetch_mode == 'cuda':
        prefetcher = CUDAPrefetcher(train_loader, opt)
        val_prefetcher = CUDAPrefetcher(val_loader, opt)
    ...
    # training
    logger.info(
        f'Start training for style:  Before Edit: {natural_name}, and After Edit : {style_name} for iter number: {total_iters}')
    start_time = time.time()


    # for epoch in range(start_epoch, total_epochs + 1):
    # sample_space_size = int(style_size[0] * style_size[1] / opt["datasets"]["val"]["sample_rate"])
    ...
    if "metrics" in opt['val']: 
        ...
        for metric, value in total_metrics.items():
            log_str += f'\t # {metric}: {value/(len(val_loader)):.4f}'
        logger = get_root_logger()
        logger.info(log_str)
